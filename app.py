...
def main():
    ...
    if menu == "ラウンド記録":
        ...
        with st.expander("📋 収支詳細"):
            ...
            for score in scores:
                hole_num = score.get("hole", 0)
                user_name = next((user["name"] for user in users if user["page_id"] == score["user_relation"]), None)
                
                if hole_num not in hole_events:
                    hole_events[hole_num] = []
                
                # オリンピックイベント
                olympic = score.get("olympic", "")
                if olympic:
                    hole_events[hole_num].append(f"🏅 {user_name}: {olympic}")
                
                # ヘビイベント
                snake = score.get("snake", 0)
                if snake > 0:
                    hole_events[hole_num].append(f"🐍 {user_name}: {snake}点")
                
                # スペシャルスコア
                par_diff = score.get("stroke", 0)
                if par_diff <= -1:
                    if par_diff == -1:
                        hole_events[hole_num].append(f"🐦 {user_name}: バーディー")
                    elif par_diff == -2:
                        hole_events[hole_num].append(f"🦅 {user_name}: イーグル")
                    elif par_diff <= -3:
                        hole_events[hole_num].append(f"⭐ {user_name}: アルバトロス")
            
            # ホール別に表示
            if hole_events:
                for hole in sorted(hole_events.keys()):
                    if hole_events[hole]:
                        st.write(f"**ホール {hole}**")
                        for event in hole_events[hole]:
                            st.write(f"  {event}")
                        st.write("")
            else:
                st.info("記録されたイベントがありません。")
            ...
            for member in game_members:
                ...
                # オリンピック詳細
                if data['olympic_score'] > 0:
                    olympic_events = [e for e in events if e["type"] == "olympic" and e["player"] == member_name]
                    olympic_detail = ", ".join([f"{e['points']}点" for e in olympic_events])
                    st.write(f"  🏅 オリンピック: +{data['olympic_score']}点 ({olympic_detail})")
                
                # スペシャル詳細
                if data['special_score'] > 0:
                    special_events = [e for e in events if e["type"] == "special" and e["player"] == member_name]
                    special_detail = ", ".join([f"{e['points']}点" for e in special_events])
                    st.write(f"  🏆 スペシャル: +{data['special_score']}点 ({special_detail})")
                
                # ヘビ詳細
                if data['snake_score'] > 0:
                    snake_events = [e for e in events if e["type"] == "snake" and e["player"] == member_name]
                    snake_detail = ", ".join([f"{e['points']}点" for e in snake_events])
                    st.write(f"  🐍 ヘビ: -{data['snake_score']}点 ({snake_detail})")
                
                # 最終収支
                balance = final_balances[member_name]
                if balance > 0:
                    st.success(f"  💰 最終収支: +{balance:.1f}点")
                elif balance < 0:
                    st.error(f"  💸 最終収支: {balance:.1f}点")
                else:
                    st.info(f"  ⚖️ 最終収支: ±0点")
            ...
            if final_transactions:
                st.subheader("🔄 最終取引")
                for transaction, amount in final_transactions.items():
